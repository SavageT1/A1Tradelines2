  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        ...
        <nav className="hidden items-center gap-7 text-base font-medium text-primary lg:flex" aria-label="Main navigation">
          <div className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
            <button className="flex items-center gap-1 transition-colors hover:text-foreground" aria-expanded={servicesOpen}>
              Tradelines <ChevronDown className="size-4" />
            </button>
            {servicesOpen && (
              <div className="absolute left-0 top-full w-72 border border-border bg-card p-2 shadow-lg">
                {serviceLinks.map((link) => (
                  <Link key={link.href} href={link.href} className="block rounded-sm px-3 py-2 text-base text-foreground transition-colors hover:bg-secondary">
                    {link.label}
                  </Link>
                ))}
              </div>
      ...
      {menuOpen && (
        <nav className="flex flex-col gap-1 border-t border-border px-6 py-4 text-base lg:hidden" aria-label="Mobile navigation">
          {serviceLinks.map((link) => (
